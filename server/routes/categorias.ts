...
categoriasRouter.get("/", (_req, res) => {
  const linhas = db
    .prepare("SELECT id, nome, cor FROM categorias ORDER BY nome COLLATE NOCASE")
    .all();
  res.json(linhas);
});
...
categoriasRouter.post("/", (req, res) => {
  ...
  try {
    const info = db
      .prepare("INSERT INTO categorias (nome, cor) VALUES (?, ?)")
      .run(parsed.data.nome, parsed.data.cor);
    const nova = db
